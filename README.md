...
* 에이전트(Agent): AI에게 '손과 발(함수/도구)'을 달아주고 알아서 목표를 달성하게 하는 패턴입니다.

* 예를 들어, 사용자가 "내 바탕화면의 `error.log` 파일을 읽고, 오늘 발생한 치명적(Fatal) 오류의 원인을 웹에서 검색해서 요약해 줘." 라고 명령하면, 에이전트는 내부적으로 `while` 루프를 돌며 다음과 같이 작동합니다.
  - Thought (생각): "먼저 바탕화면의 `error.log` 파일을 읽어야겠다."
  - Action (행동): C++이나 파이썬으로 짜인 `read_file("C:/Desktop/error.log")` 함수를 호출합니다.
  - Observation (관찰): 함수가 리턴한 텍스트(로그 내용)를 읽습니다. "아, 메모리 누수 에러 코드가 `0x80004005`로 찍혀 있네."
  - Thought (생각): "이제 이 에러 코드를 구글에 검색해야겠다."
  - Action (행동): `web_search("error code 0x80004005")` 함수를 호출합니다.
  - Observation (관찰): 웹 검색 결과를 확인합니다.
  - Final Answer (최종 답변): 모든 정보를 종합하여 사용자에게 최종 요약본을 출력하고 루프를 탈출(`break`)합니다.

* LLM이 스스로 어떤 함수를 호출할지 결정하고 실행 결과를 받아 다음 행동을 계획하는 것이 에이전트의 핵심입니다.


## MCP(Model Context Protocol)란 무엇인가?

* MCP (Model Context Protocol): AI를 위한 만능 USB-C 케이블
  - 에이전트가 쓸 수 있는 도구(로컬 파일 읽기, 웹 검색, 슬랙 메시지 보내기 등)를 만들었다고 가정해 봅시다.
  - 과거에는 OpenAI, Claude, 구글 Gemini 등 각 AI 모델마다 이 도구를 연결하는 API 규격이 전부 달랐습니다. 새로운 AI 모델이 나오면 코드를 처음부터 다시 짜야 했죠.
  - 이를 해결하기 위해 Anthropic(Claude 개발사)이 주도하여 만든 오픈 소스 표준 프로토콜이 바로 MCP입니다.

* MCP는 철저한 클라이언트-서버 아키텍처를 따릅니다.
  - MCP 호스트 (클라이언트): AI 모델(LLM)이 돌아가고 있는 챗봇 UI나 에이전트 프로그램입니다.
  - MCP 클라이언트 (미들웨어): 호스트 내부에 존재하며, 서버들과 1:1 연결(주로 표준 입출력 stdio나 SSE 통신)을 맺습니다.
  - MCP 서버: 특정한 도구나 데이터를 제공하는 독립적인 프로그램입니다. (예: 'GitHub MCP 서버', '구글 드라이브 MCP 서버', '로컬 파일 시스템 MCP 서버')


## MCP, 에이전트, RAG를 융합하여 나만의 AI 비서 만들어보기

```
[ AI 모델 (두뇌 역할: Gemini API, 로컬 Llama 등) ]
         ^
         | 1. API 통신 (HTTP REST / WebSockets)
         v
[ MCP 호스트 (에이전트 / 어플리케이션 본체) ]  <-- (내부에 [ MCP 클라이언트 ] 모듈 포함)
         ^
         | 2. 표준 입출력(stdio) / SSE 통신 등
         v
[ MCP 서버 (손발 역할: 로컬 파일 제어, DB 검색 등 실제 도구) ]
```

* 각 계층의 역할은 다음과 같습니다.
  - AI 모델 (두뇌): 클라우드(구글, OpenAI 서버)에 있거나 로컬 서빙 엔진(Ollama 등)에 떠 있는 순수한 지능입니다. 텍스트를 생성하고 "어떤 함수를 써야 할지" 판단만 내립니다.
  - MCP 호스트 (프로그램 본체): AI 모델과 통신하며 전체 흐름을 제어하는 실제 '소프트웨어 애플리케이션'입니다. (예: 배포된 Claude Desktop 앱, 또는 직접 개발하신 데스크톱 비서 프로그램)
  - MCP 클라이언트 (통신 미들웨어): 보통 MCP 호스트 프로그램 '내부'에 라이브러리 형태로 포함됩니다. AI가 내린 "도구를 쓰겠다"는 명령을 MCP 서버가 알아들을 수 있는 JSON-RPC 표준 규격으로 번역하고 연결을 관리합니다.
  - MCP 서버 (도구 프로세스): 실제로 PC에서 코드를 실행하고, 파일을 읽거나, 웹 검색을 수행하는 독립적인 실행 파일이나 스크립트들입니다.

* 정리하자면, MCP 호스트(애플리케이션)가 시스템의 중앙에 위치하여, 위로는 API를 통해 AI(두뇌)의 지시를 받고, 아래로는 표준 입출력 등을 통해 MCP 서버(손발)를 조종하는 오케스트레이터 역할을 수행하는 구조입니다.
  - '호스트'가 회사의 CEO라면, '서버'는 업무를 처리하는 각 부서라고 할 수 있습니다.
  - CEO가 각 부서들과 연결되려면 부서별 전담 비서가 필요해요. 이 비서들이 바로 '클라이언트'입니다.
  - 여기서 MCP란 업무 요청과 회신 내용을 정리하는 문서의 '양식'과 같습니다. 문서에 요청사항, 담당 부서, 요청 형식, 작업 기한 등의 항목을 통일해서 사용한다면 실무자들이 CEO의 요청사항을 한번에 알아듣고 수행할 수 있겠죠.

* 지금까지 배운 AI, 에이전트, RAG, MCP가 하나의 시스템으로 어떻게 결합되는지 표현하면 다음과 같은 아키텍처 다이어그램이 나옵니다.

```
[ AI 모델 (두뇌: Gemini API) ]
         ^
         | 1. 프롬프트 전송 & 도구 사용 결정 (JSON)
         v
====================================================================
[ MCP 호스트 (에이전트 본체 / 예: C# 데스크톱 앱) ]
  * 에이전트(Agent) 로직: 
    - 사용자의 명령을 받음
    - "무엇을 해야 할지" AI에게 묻고 (Reason),
    - AI의 결정에 따라 아래의 서버들을 호출 (Act)하는 무한 루프(`while`)
====================================================================
         ^
         | 2. 표준 입출력 (JSON-RPC)을 통한 도구 호출
         v
[ MCP 서버들 (손발 및 오픈북) ]
  ├── [ 일반 MCP 서버 ] : 로컬 파일 읽기, PC 시스템 제어 도구
  ├── [ 네트워크 MCP 서버 ] : 사내 SQL DB 접근, API 호출 도구
  └── [ RAG MCP 서버 ] : Vector DB에서 보안 매뉴얼/문서 검색 도구
```
